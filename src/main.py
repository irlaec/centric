...
def get_stats_for_coupons_by_webshop_id(coupons_df: pd.DataFrame) -> dict:
    coupons_by_webshop_id = coupons_df.groupby(['promotion_type', 'webshop_id'])['value']
    ...
    for (promotion_type, webshop_id), stats in coupons_by_webshop_id_stats.iterrows():
        if stats["count"]:
            ...
            results[f"{webshop_id}/{promotion_type}"] = describe_series_to_dict_simplified(stats)
    return results
...
if __name__ == '__main__':
    ...
    plot_percent_off_and_dollar_off_stats(results["percent_off_coupon_stats"], results["dollar_off_coupon_stats"])
