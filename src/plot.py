...
def plot_num_coupons_per_type(num_coupons_per_type: dict):
    ...
    sns.barplot(
        x=types, y=counts, hue=types, palette="viridis", dodge=False, legend=False
    )
...
def plot_percent_off_and_dollar_off_stats(
    percent_off_stats: dict, dollar_off_stats: dict
):
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(
        stats_names, percent_off_values, color="skyblue", alpha=0.5, label="Percent-off"
    )
    plt.bar(
        stats_names, dollar_off_values, color="orange", alpha=0.5, label="Dollar-off"
    )
